...
def quote():
    ...
    if request.method == "GET":
        ...
        quote_sym = request.form.get("symbol")
        quote = lookup(quote_sym)
        if not quote:
...
def sell():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            ...
            return apology("must provide a positive integer number of shares", 403)
        else:
            shares = int(shares)
        ...
        for stock in stocks:
            if stock["symbol"] == symbol:
                if stock["total_shares"] < shares:
                    ...
                    # adding the sale to history table
                    db.execute(
                        "INSERT INTO transactions (user_id,symbol,shares,price) VALUES (:user_id, :symbol, :shares, :price)",
                        user_id=session["user_id"],
                        symbol=symbol,
